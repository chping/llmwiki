...
from tools.common import WIKI_DIR, append_log
...
def sync_index() -> Path:
    ...
    for path in sorted(WIKI_DIR.rglob("*.md")):
        ...
        if rel.name in SKIP:
            continue
        if "template.md" in rel.parts:
            continue
        group = rel.parts[0] if len(rel.parts) > 1 else "pages"
        pages[group].append(rel)

    block_lines = ["<!-- AUTO-GENERATED:START -->", ""]
    for group in sorted(pages):
        block_lines.append(f"## {group}")
        block_lines.append("")
        for rel in pages[group]:
