...
3. 提炼文件的摘要和核心内容，更新 `wiki/pages` 目录下同名页面的内容
4. 提炼文件涉及的主题，总结每个主题的摘要和核心内容，更新 `wiki/topics` 目录下相关主题页面内容
...
## 文章页面维护规则（Pages Rules）

- 以原始文件名（不含后缀）作为页面唯一标识，在 `wiki/pages/` 目录下查找对应的 `.md` 文件：
  - 若存在同名页面，必须优先更新该页面，禁止创建重复页面
  - 若不存在同名页面，则创建新的文章页面
- 每个页面必须与 `raw/` 目录中的原始文件保持一一对应关系（同名映射）
- 在页面中写入对原始文件的结构化内容提炼，包括但不限于：
  - 摘要（Summary）
  - 核心内容（Key Points）
  - 重要结论或数据（如适用）
- 页面内容必须仅围绕单个原始文件展开，不得混入其他来源的未标注信息
- 创建或更新页面时，必须严格遵循模板：`wiki/pages/template.md` 中定义的结构和层级要求

## 主题页面维护规则（Topics Rules）
